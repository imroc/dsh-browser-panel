...
function sessionOf(exec) {
  ...
  if (typeof sessionId !== 'string' || sessionId === '') {
    throw new Error('browser_panel_* tools require a calling agent session; this call has no owning session')
  }
  return sessionId
}
...
export function defineTools({ manager, human, hub, config, getAttachments, logger, getPanelAvailable = () => true }) {
  ...
  const navigate = defineTool({
    ...
    description:
      "Open a URL in this session's browser tab (starting the browser if needed). Each session has its own tab, and the human can watch or take over yours from the DSH Web UI panel. All sessions share one login profile, so a login done once works everywhere.",
    parameters: {
  ...
  const snapshot = defineTool({
    ...
    description:
      "Read this session's page as structured text: title, URL, a numbered inventory of clickable/typable elements, and the visible text. Pass an element number to browser_panel_click or browser_panel_type.",
    parameters: {
  ...
  const click = defineTool({
    ...
    description:
      "Click something in this session's page: either an element number from browser_panel_snapshot, or a visible text label to match. Real input events are dispatched, so hover/focus behaviour matches a human click.",
    parameters: {
  ...
  const screenshot = defineTool({
    ...
    description:
      "Capture this session's page as an image so you can see the rendering (layout, CAPTCHA, QR codes, charts). Prefer browser_panel_snapshot for reading text.",
    parameters: {
  ...
  const close = defineTool({
    ...
    description:
      "Close this session's browser tab and free its memory. The shared profile keeps every login, so the next call opens a fresh tab already authenticated. The human can close your tab from the panel too, in which case the next tool call simply opens a new one.",
    parameters: {},
    output: jsonOutput(),
