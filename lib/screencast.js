...
/** A connected panel with no frame for this long counts as a stalled stream. */
const STALL_MS = 8000
...
export class ScreencastHub {
  ...
  async pushState({ force = false } = {}) {
    ...
    // Self-heal: an injected click is silently dropped while the page sits in a
    // background tab (a stray tab, a page that opened one itself), so take the
    // foreground back whenever the poll notices we lost it.
    if (status.running === true && status.hidden === true) {
      this.logger?.debug?.('shared page fell behind another tab — reactivating it')
  ...
  async startStreaming() {
    // A disconnecting panel stops the stream asynchronously; starting before it
    // finished would leave the new screencast killed by the old stop (the panel
    // would then sit on a frozen frame while state says "streaming").
    if (this.stopping !== undefined) await this.stopping.catch(() => {})
  ...
  /** Release the screencast resources of the stream that just ended. */
  async finishStopping() {
    ...
    if (page !== undefined) {
      ...
      // would flip the page between two sizes on every panel reconnect, and a
      // panel mid-mapping would then aim at the wrong coordinates.
  ...
  /**
   * Self-heal a stalled stream: Chrome stops emitting frames if an
   * acknowledgement is ever lost, and a panel would then sit on a frozen image.
   */
  checkStreamHealth() {
    if (!this.streaming || this.connections.size === 0) return
    if (Date.now() - this.lastFrameAt < STALL_MS) return
    this.logger?.warn?.('screencast stalled — restarting the stream')
    const page = this.page
    this.streaming = false
    this.stopping = (async () => {
      for (const off of this.detach) off()
      ...
      clearInterval(this.stateTimer)
      this.stateTimer = undefined
      if (page !== undefined) await page.send('Page.stopScreencast').catch(() => {})
    })()
