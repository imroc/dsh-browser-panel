...
export class BrowserManager {
  ...
  async status() {
    ...
    try {
      const info = await this.page.evaluate(
        'JSON.stringify({url: location.href, title: document.title, w: innerWidth, h: innerHeight, hidden: document.visibilityState === "hidden"})',
      )
  ...
  async start() {
    ...
    // Chrome drops injected input for a background tab, and a panel would look
    // alive while every click went nowhere — always own the foreground tab.
    await this.cdp.activate(this.page.targetId).catch(() => {})
  ...
  /** Bring the shared page back to the foreground when something else stole it. */
  async ensureActive() {
    if (this.page === undefined || this.cdp === undefined) return false
    await this.cdp.activate(this.page.targetId).catch(() => {})
    return true
  }
